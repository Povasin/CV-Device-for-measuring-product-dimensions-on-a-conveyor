...
class SqliteOutbox:
    ...
    def enqueue(self, message: MeasurementMessage) -> bool:
        """Store a message once; return false for an existing idempotency key."""
        cursor = self._connection.execute(
            ...
            VALUES (?, ?, ?, ?, ?, ?)
            """,
            (message.measurement_id, *message.dimensions_mm, message.algorithm, message.calibration_id),
        )
        self._connection.commit()
    ...
    def mark_delivered(self, measurement_id: str) -> None:
        ...
        self._connection.execute("UPDATE outbox SET delivered = 1 WHERE measurement_id = ?", (measurement_id,))
        self._connection.commit()
